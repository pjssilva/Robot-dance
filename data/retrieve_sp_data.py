...
sp = pd.read_csv("dados_covid_sp.csv", sep=";", parse_dates=[4])
...
sp = sp.loc[: ,["nome_munic", "datahora", "casos", "pop"]]
sp["state"] = "SP"
sp.columns = ["city", "date", "confirmed", "estimated_population_2019", "state"]
sp.to_csv("covid_with_cities.csv")

for d in pop_drs.index:
    drs.loc[d, "pop"] = pop_drs[d]
drs.reset_index(inplace=True)
drs = drs.loc[: ,["nome_drs", "datahora", "casos", "pop"]]
drs["state"] = "SP"
drs.columns = ["city", "date", "confirmed", "estimated_population_2019", "state"]
